...
export const PricingCard = ({
    planName, description, price, features, buttonText, isPopular = false, buttonVariant = 'primary', href = "/booking"
}: PricingCardProps) => {
    const cardClasses = `
    ...
    dark:from-black/80 dark:to-black/60 dark:border-white/10 dark:backdrop-brightness-[0.91]
    ${isPopular ? 'scale-105 relative ring-2 ring-[#FDB813]/50 shadow-2xl' : ''}
  `;
    const buttonClasses = `
    mt-auto w-full py-2.5 rounded-xl font-bold text-[14px] transition font-sans flex items-center justify-center
    ${buttonVariant === 'primary'
            ? 'bg-[#FDB813] hover:bg-[#e5a700] text-black'
  ...
  `;
    ...
    return (
        <div className={cardClasses.trim()}>
            {isPopular && (
                <div className="absolute -top-4 right-4 px-3 py-1 text-[12px] font-bold rounded-full bg-[#FDB813] text-black">
                    Most Popular
                </div>
            )}
            <div className="mb-3">
                <h2 className="text-[32px] font-bold tracking-tight text-white font-display">{planName}</h2>
                <p className="text-[14px] text-white/80 mt-1 font-sans">{description}</p>
            </div>
            <div className="my-6 flex items-baseline gap-2">
                <span className="text-[48px] font-extralight text-white font-display">${price}</span>
                <span className="text-[14px] text-white/80 font-sans">/course</span>
            </div>
            <div className="card-divider w-full mb-5 h-px bg-[linear-gradient(90deg,transparent,rgba(255,255,255,0.1)_50%,transparent)] dark:bg-[linear-gradient(90deg,transparent,rgba(255,255,255,0.09)_20%,rgba(255,255,255,0.22)_50%,rgba(255,255,255,0.09)_80%,transparent)]"></div>
            <ul className="flex flex-col gap-2 text-[14px] text-white/90 mb-6 font-sans">
                {features.map((feature, index) => (
                    <li key={index} className="flex items-center gap-2">
                        <CheckIcon className="text-[#FDB813] w-4 h-4" /> {feature}
                    </li>
                ))}
            </ul>
...
interface ModernPricingPageProps {
    ...
    /** Optional background image URL to use instead of the shader */
...
export const ModernPricingPage = ({
    ...
    return (
        <div className="bg-background text-foreground w-full overflow-x-hidden relative py-20">
            {backgroundImage ? (
            ...
            <main className="relative w-full flex flex-col items-center justify-center px-4 z-10">
                <div className="w-full max-w-5xl mx-auto text-center mb-14">
                    <h1 className="text-[48px] md:text-[64px] font-extralight leading-tight tracking-[-0.03em] text-white font-display">
                        {title}
                    </h1>
