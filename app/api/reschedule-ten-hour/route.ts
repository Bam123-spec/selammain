...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!durationMinutes) durationMinutes = Number(pkg?.duration_minutes || 120);
    ...
    while (isBefore(currentSlotUTC, dayEndLimitUTC)) {
      ...
      if (isBefore(minNoticeTime, currentSlotUTC)) {
        ...
        if (!overlaps && !inBreak) slots.push(currentSlotUTC.toISOString());
      }
    ...
    try {
      ...
      if (studentEmail) {
        ...
        await sendBrevoEmail({
          ...
          subject: "Booking Rescheduled - 10-Hour Package Session",
          htmlContent: generateStudentBookingEmail({
            name: studentName,
            email: studentEmail,
            service: "10-Hour Package Session (Rescheduled)",
            date: dateFormatted,
            time: timeFormatted,
      ...
      await sendBrevoEmail({
        ...
        subject: `Rescheduled: 10-Hour Package Session - ${studentName}`,
      ...
      console.error("Failed to send 10-hour reschedule email:", emailError);
    }

    return NextResponse.json({ success: true });
  } catch (error: any) {
    console.error("10-hour reschedule API error:", error);
