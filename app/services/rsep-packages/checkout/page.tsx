...
export default function RsepEmbeddedCheckoutPage() {
    const searchParams = useSearchParams();
    const [clientSecret, setClientSecret] = useState<string | null>(null);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState<string | null>(null);

    const serviceSlug = (searchParams.get("service_slug") || "rsep").trim().toLowerCase();
    ...
    const backHref = useMemo(() => {
        if (location === "bethesda") {
            return "/services/rsep-packages?location=bethesda";
        }
        return "/services/rsep-packages";
    }, [location]);

    const className = searchParams.get("class_name") || "3-Hour Roadway Safety Education Program (RSEP)";
