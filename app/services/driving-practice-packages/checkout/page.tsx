...
function formatDisplayTime(value: string, durationHours: number) {
    const trimmed = (value || "").trim();
...
export default function DrivingPracticeEmbeddedCheckoutPage() {
    const searchParams = useSearchParams();
    const [clientSecret, setClientSecret] = useState<string | null>(null);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState<string | null>(null);

    const serviceSlug = (searchParams.get("service_slug") || "driving-practice-1hr").trim().toLowerCase();
    ...
    const isTwoHour = serviceSlug === "driving-practice-2hr";
    const className = searchParams.get("class_name") || (isTwoHour ? "Driver's Practice (2 Hour)" : "Driver's Practice (1 Hour)");
    const classDate = searchParams.get("class_date") || "";
    const classTime = searchParams.get("class_time") || "";
    const displayTime = formatDisplayTime(classTime, isTwoHour ? 2 : 1);
    const returnPath = `/checkout/success?service_slug=${encodeURIComponent(serviceSlug)}`;
    const backHref = isTwoHour ? "/booking?plan=driving-practice-2hr" : "/booking?plan=driving-practice-1hr";
    const backLabel = isTwoHour ? "Back to 2-Hour Practice" : "Back to 1-Hour Practice";
